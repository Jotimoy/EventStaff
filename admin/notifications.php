...
if ($action === 'read_all') {
    $notifier->markAllAsRead($admin_id);
    header('Location: notifications.php');
    exit();
}
...
<head>
    ...
    <title>Notifications - EventStaff Admin</title>
    ...
    <style>
        * {
        ...
        .icon-system { background: rgba(23, 162, 184, 0.1); color: #17a2b8; }
        .icon-payment { background: rgba(255, 193, 7, 0.1); color: #ff9800; }
        .icon-user { background: rgba(102, 126, 234, 0.1); color: var(--primary); }
    ...
    <nav class="navbar navbar-expand-lg navbar-dark" style="background: linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%); box-shadow: 0 2px 8px rgba(0,0,0,0.1); position: fixed; top: 0; width: 100%; z-index: 1000;">
        <div class="container-lg">
            <a class="navbar-brand fw-bold" href="dashboard.php">
                <i class="bi bi-briefcase me-2"></i>EventStaff Admin
            </a>
    ...
    <div class="page-header">
        <div class="container-lg">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1><i class="bi bi-bell-fill me-2"></i>System Notifications</h1>
                    <p>Monitor all system alerts and important events</p>
                </div>
    ...
    <div class="container-lg mb-5" style="max-width: 800px;">
        ...
        <?php if (empty($notifications)): ?>
            <div class="empty-state">
                ...
                <p>System alerts, payment notifications, and user activity updates will appear here</p>
                <a href="dashboard.php" class="btn-mark-read" style="display: inline-block; margin-top: 20px;">
                    <i class="bi bi-graph-up me-1"></i>Back to Dashboard
                </a>
            </div>
        <?php else: ?>
            <div>
                <?php foreach ($notifications as $notification): ?>
                    <div class="notification-item <?php echo strtolower($notification['status']); ?> <?php 
                        if (strpos($notification['type'], 'payment') !== false) echo 'payment';
                        elseif (strpos($notification['type'], 'system') !== false) echo 'system';
                        else echo 'system';
                    ?>">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            <div style="display: flex; align-items: start; flex: 1;">
                                <div class="notification-icon <?php
                                    if (strpos($notification['type'], 'payment') !== false) echo 'icon-payment';
                                    else echo 'icon-system';
                                ?>">
                                    <?php if (strpos($notification['type'], 'payment') !== false): ?>
