...
if ($action === 'read_all') {
    $notifier->markAllAsRead($employee_id);
    header('Location: notifications.php');
    exit();
}
        ...
        <div class="container-lg">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p>Stay updated with important platform events</p>
                </div>
    ...
    <div class="container-lg mb-5" style="max-width: 800px;">
        ...
        <?php if (empty($notifications)): ?>
            <div class="empty-state">
                ...
                <p>All your notifications will appear here. Start applying for shifts to get updates!</p>
                <a href="shifts.php" class="btn-mark-read" style="display: inline-block; margin-top: 20px;">
                    <i class="bi bi-calendar-event me-1"></i>Browse Shifts
                </a>
            </div>
        <?php else: ?>
            <div>
                <?php foreach ($notifications as $notification): ?>
                    <div class="notification-item <?php echo strtolower($notification['status']); ?> <?php 
                        if (strpos($notification['type'], 'approved') !== false) echo 'approved';
                        elseif (strpos($notification['type'], 'rejected') !== false) echo 'rejected';
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            <div style="display: flex; align-items: start; flex: 1;">
                                <div class="notification-icon <?php
                                    if (strpos($notification['type'], 'approved') !== false) echo 'icon-approved';
                                    elseif (strpos($notification['type'], 'rejected') !== false) echo 'icon-rejected';
                                    elseif (strpos($notification['type'], 'payment') !== false) echo 'icon-payment';
                                ?>">
                                    <?php if (strpos($notification['type'], 'approved') !== false): ?>
                                        <i class="bi bi-check-circle-fill"></i>
                                    <?php elseif (strpos($notification['type'], 'rejected') !== false): ?>
